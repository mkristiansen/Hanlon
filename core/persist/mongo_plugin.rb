...
module ProjectHanlon
  module Persist
    ...
    class MongoPlugin < PluginInterface
      ...
      # Checks whether DB 'ProjectHanlon' is selected in MongoDB
      #
      # @return [Boolean] Connection status
      #
      def is_db_selected?
        #logger.debug "Is ProjectHanlon DB selected?(#{(@hanlon_database != nil and @connection.active?)})"
        (@hanlon_database != nil and @connection.active?)
      end
      ...
      def collection_by_name(collection_name)
        if is_db_selected?
          @hanlon_database.collection(collection_name.to_s)
        else
